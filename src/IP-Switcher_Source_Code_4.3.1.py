...
def enable_dhcp(interface_var, ip_current, subnet_current, gateway_current, interfaces_cache):
    ...
    try:
        ...
        messagebox.showinfo("DHCP Enabled", f"DHCP has been enabled for {interface_name}. Refresh to see IP assignment.")

        refresh_interfaces()

    except subprocess.CalledProcessError as e:
        messagebox.showerror("Error", f"Failed to enable DHCP: {str(e)}")
...
def change_ip(interface_var, ip_entry, subnet_entry, gateway_entry, ip_tree, ping_interval_entry, i, ip_current, subnet_current, gateway_current, interfaces_cache):
    ...
    # Check if the IP is already assigned to another interface
    for interface in interfaces_cache:
        if interface['name'] != interface_name and interface['ip'] == ip:
            # Notify user that the IP is already in use by another interface
            messagebox.showerror("Error", f"The IP address {ip} is already in use by the interface '{interface['name']}'. Please choose a different IP.")
            return
...
def show_about_info():
    messagebox.showinfo("About", "This application was developed to manage IP configurations. For internal use in Trafsys AS only.\nVersion 4.3.1")
